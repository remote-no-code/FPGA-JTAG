...
initial begin
    ...
    debug_ctrl_reg = 32'h00000000;
end

always @(posedge tck or posedge trst) begin
	$display("%0t TAP state = %0d", $time, state);
...
always @(posedge tck) begin
	...
	if(state == CAPTURE_DR) begin
		$display("%0t CAPTURE_DR state=%0d ir=%0d debug_pc=%08h",
             $time, state, ir, debug_pc);
		case(ir)
		...
		DEBUG_STATUS: begin
		    //$display("%0t CAPTURE_DR STATUS debug_halted=%b", $time, debug_halted);
		    $display("%0t Loading debug_pc=%08h into dr_shift",
            	 $time, debug_pc);
		    dr_shift <= {31'b0, debug_halted};
		    //dr_shift <= debug_ctrl_reg;
		end
	...
	endcase
		...
		$display("%0t SHIFT_DR dr_shift=%08h",
         	$time, dr_shift);
     	$display("%0t SHIFT_DR dr_shift=%08h  tdo_reg=%b",
         	$time, dr_shift, tdo_reg);
		case (ir)
...
always @(posedge tck) begin
    ...
    if(state == UPDATE_DR && ir == DEBUG_CTRL) begin
        $display("TAP CTRL = %h", dr_shift);
        debug_ctrl_reg <= dr_shift;

        debug_halt_req   <= dr_shift[0];
        $display("%0t TAP: halt_req=%b", $time, dr_shift[0]);
...
always @(posedge tck) begin
    if (state == UPDATE_DR && ir == DEBUG_CTRL) begin
        if (dr_shift[2])
            $display("%0t TAP: RESET request", $time);
    end
end

always @(posedge tck) begin
    if (state == UPDATE_DR) begin
    	$display("%0t UPDATE_IR ir_shift=%b (%0d)", $time, ir_shift, ir_shift);
        $display("UPDATE_DR ir=%h dr=%h", ir, dr_shift);

        if (ir == DEBUG_CTRL)
            $display("DEBUG_CTRL write = %h", dr_shift);
    end
end
endmodule
