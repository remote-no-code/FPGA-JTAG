...
//---------------------------------------------------------
// Toggle generation in TCK domain
//---------------------------------------------------------
...
begin
    ...
    begin
        ...
        if (halt_sync2 ^ halt_sync2_d)
            $display("%0t : HALT pulse", $time);

        if (resume_sync2 ^ resume_sync2_d)
            $display("%0t : RESUME pulse", $time);

        if (reset_sync2 ^ reset_sync2_d)
