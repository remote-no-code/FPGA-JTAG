...
always @(posedge tck or negedge n_trst) begin
    if (!n_trst) begin
        ...
        case (state)
            CAPTURE_IR: ir_shift <= IDCODE;
            SHIFT_IR:   ir_shift <= {tdi, ir_shift[4:1]};
...
// Data register and DMI interface
always @(posedge tck or negedge n_trst) begin
    if (!n_trst) begin
        dr_shift   <= 41'd0;
        bypass_bit <= 1'b0;
        ...
        case (state)
            CAPTURE_DR: begin
                case (ir)
                    ...
                    DMI:    dr_shift <= {7'd0, dmi_rdata_reg, dmi_resp_reg};
            ...
            UPDATE_DR: begin
                ...
                case (ir)
                    ...
                    DMI: begin
                        if (req_ready) begin
                            dmi_addr  <= dr_shift[40:34];
                            dmi_wdata <= dr_shift[33:2];
                            dmi_op    <= dr_shift[1:0];
                            dmi_valid <= 1'b1;
                        end
                    end
...
always @(posedge tck or negedge n_trst) begin
    ...
    else if (state == UPDATE_DR && ir == DTMCS && dr_shift[16]) begin
        dmi_rdata_reg <= 32'd0;
        dmi_resp_reg  <= 2'b00;
    end
...
always @(posedge tck or negedge n_trst) begin
    if (!n_trst)
        ...
        case (ir)
            IDCODE, DTMCS, DMI: tdo_next <= dr_shift[0];
            BYPASS:             tdo_next <= bypass_bit;
