...
`ifdef DISPLAY
    $display("%0t DMI_IF SEND addr=%02h op=%0d data=%08h",
             $time,
             dmi_addr,
             dmi_op,
             dmi_wdata);
...
`ifdef DISPLAY
                $display("====================================");
                $display("%0t DMI_IF RECEIVED", $time);
                $display("ADDR = %02h", dmi_addr);
                $display("OP   = %0d", dmi_op);
                $display("DATA = %08h", dmi_wdata);
`endif
...
`ifdef DISPLAY
                $display("====================================");
                $display("%0t DTM RECEIVE", $time);
                $display("READY1=%b", dmi_ready);
                $display("RDATA1=%08h", dmi_rdata);
                $display("RESP_1=%0d", dmi_resp);
`endif
...
`ifdef DISPLAY
                $display("====================================");
                $display("%0t DMI RESPONSE", $time);
                $display("READY2=%b", dm_ready);
                $display("RDATA2=%08h", dm_rdata);
                $display("RESP_2=%0d", dm_resp);
`endif
...
always @(posedge clk) begin
    $display("%0t DMI_IF state=%0d dmi_valid=%b dm_valid=%b dm_ready=%b",
             $time,
             state,
             dmi_valid,
             dm_valid,
